...
ADMIN/FORENSICS STORE (slice 12, `WebhookDeliveryAdminStore`): the deliveries LIST + manual RETRY surface for the dev portal. Reusable facts:
- webhook_deliveries has NO tenant_id of its own → tenant isolation is the EXPLICIT predicate `(@p_tenant::uuid IS NULL OR s.tenant_id = @p_tenant::uuid)` joined through `webhook_subscriptions.tenant_id`, present on ALL THREE methods. This is the canonical no-RLS-table isolation pattern here.
- tenantScope is ALWAYS `ctx.ImpersonatedTenantId ?? ctx.TenantId` (JWT-only; `RequestContext.cs:43,62`). NEVER a path/body/header. A non-null scope can't match the NULL platform channel.
- `platform.api_clients.manage` = scope='platform', is_dangerous=true, granted ONLY to super_admin by default seed (`01_platform_core.sql:303-307`; tenant_owner/admin/viewer excluded by scope filter, platform_support excluded by read-only filter). So the null-scope→all-tenants branch is the intended super_admin posture.
- Metadata-only DTO `WebhookDeliveryDto` (`WebhookDtos.cs:45`) NEVER carries payload/response_body/response_headers/signature. error_message = transport only (WebhookHttpDispatcher returns `HTTP {code}`/ex.Message, never reads the response body) → no PHI.
- Secret commands (Register/Rotate/CreateWebhook) marked `IDoNotCacheResponse` → `Behaviors.cs:138` bypasses the plaintext idempotency store entirely (no serve, no save). Tradeoff: idempotency replay protection is lost for these writes (a same-key retry re-executes — duplicate webhook / double-rotate possible). Not a security leak; cleaner fix would be redact-on-cache.
- CreateWebhook binds tenant from JWT and IGNORES body TenantId (`command.Request with { TenantId = ctx.ImpersonatedTenantId ?? ctx.TenantId }`) — closes the subscribe-to-all spoof; legit platform NULL channel still available to a no-tenant super_admin.
