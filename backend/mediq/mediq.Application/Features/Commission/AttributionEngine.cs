    ...
    : ICommandHandler<CreateAttributionCommand, AttributionResultDto>
{
    public async Task<AttributionResultDto> Handle(CreateAttributionCommand command, CancellationToken ct)
    {
        ...
        var attribution = Attribution.Create(
            ...
            matched?.RuleId, commission, score, flags, now);
        ...
        await audit.RecordAsync(new AuditEntry(
            ...
            ChangeSummary: $"Attribution via {req.AttributionSource}; commission ₹{commission}; fraud {score:0.000} [{string.Join(',', flags)}]"), ct);

        // Integration event — IDs + amount ONLY, NO patient PHI.
        await events.PublishAsync("commission.attribution.created", command.TenantId,
            new { attribution_id = attribution.AttributionId, broker_id = req.BrokerId, booking_id = req.BookingId, commission_inr = commission }, ct);

        return new AttributionResultDto(
            attribution.AttributionId, req.BookingId, req.BrokerId, req.AttributionSource,
            verification, attribution.CommissionStatus, commission, score, flags);
    }
}
