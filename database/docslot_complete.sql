...
-- A background worker has NO tenant/request context, so it reaches the RLS-protected bookings through
-- these definer functions (the rls-cross-tenant-worker pattern). They expose ONLY non-PHI booking
-- metadata — the ids + the lead-time / slot-hour / on-behalf FEATURES (identical to
-- BookingReadService.GetNoShowFeaturesAsync) — NEVER patient identity or any PHI. The worker mints a
